...
if (!isset($_FILES['foto']) || $_FILES['foto']['error'] !== UPLOAD_ERR_OK) {
    header("Location: /netcare/opd/dashboard_opd.php?page=profil_akun&error=upload");
    exit;
}
...
if (!isset($allowed[$mime])) {
    header("Location: /netcare/opd/dashboard_opd.php?page=profil_akun&error=format");
    exit;
}

/* 🔥 LIMIT 2MB */
if ($file['size'] > 2 * 1024 * 1024) {
    header("Location: /netcare/opd/dashboard_opd.php?page=profil_akun&error=size");
    exit;
}
...
/* SUCCESS */
header("Location: /netcare/opd/dashboard_opd.php?page=profil_akun&success=foto");
exit;