...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['ajax'] ?? '') === 'upload') {
    ...
    try {
        ...
        if ($pengajuan_id <= 0 || $nama === '' || $deskripsi === '') {
        ...
        foreach ($files['name'] as $i => $name) {
            ...
            if (move_uploaded_file($files['tmp_name'][$i], $target)) {
                $stmt = $conn->prepare("INSERT INTO dokumentasi 
                    (pengajuan_id, nama, deskripsi, file_path, tanggal_upload)
                    VALUES (?, ?, ?, ?, NOW())");
                $stmt->bind_param("isss", $pengajuan_id, $nama, $deskripsi, $relPath);
                $stmt->execute();
...
if ($_SERVER['REQUEST_METHOD'] === 'GET' && ($_GET['ajax'] ?? '') === 'tabel') {
    ...
    $res = $conn->query("
        SELECT 
            ...
            p.nama AS judul_pengajuan,
            GROUP_CONCAT(DISTINCT d.nama SEPARATOR ', ') AS judul_dok,
...
$pengajuan = $conn->query("SELECT id,nama FROM pengajuan ORDER BY id DESC");
?>
<!DOCTYPE html>
<html lang="id">
<head>
...
<body>
<div class="container">
    <h2 style="text-align:center;">REKAPITULASI DOKUMENTASI KEGIATAN</h2>
...
<script>
const ajaxURL = '<?= $APP_ROOT ?>/admin/dokumentasi.php';
